...
export async function GET(req: NextRequest, { params }: { params: Promise<{ mediaType: MediaType }> }) {
    ...
    try {
        ...
        const response = await fetch(`${TMDB_CONFIG.baseUrl}/${mediaType}/popular?${queryString}`, {
        ...
        return NextResponse.json({ error: `Failed to fetch popular ${mediaType}` }, { status: 500 })
    }
}
